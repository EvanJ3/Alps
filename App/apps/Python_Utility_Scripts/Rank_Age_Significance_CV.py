...
def run():
    ...
    for i in range(0,len(gender_options)):
        ...
        for j in range(0,len(event_options)):
            ...
            Full_cv_results = Indv_Ranking_KDE_CV(df_copy,genders=genders_i,events=events_j,rank_type='Adj_Rank')
            Full_optimal_kernel,Full_optimal_bw = Parse_Optimal_Parameters_From_CV_Results(Full_cv_results)

            Top_cv_results = Indv_Ranking_KDE_CV(df_copy,genders=genders_i,events=events_j,rank_type='Scaled_Rank')
            Top_optimal_kernel,Top_optimal_bw = Parse_Optimal_Parameters_From_CV_Results(Top_cv_results)

            Full_optimal_kde = Generate_Indv_Rank_KDE_Fit(df=df_copy,genders=genders_i,events=events_j,mode=1,kernel=Full_optimal_kernel,bandwidth=Full_optimal_bw)
            Top_optimal_kde = Generate_Indv_Rank_KDE_Fit(df=df_copy,genders=genders_i,events=events_j,mode=2,kernel=Top_optimal_kernel,bandwidth=Top_optimal_bw)
    ...
    result_df.to_csv(DATA_PATH.joinpath('INDV_KDE_RESULT_DF.csv'),index=False)
